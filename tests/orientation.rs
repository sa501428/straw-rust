...
fn transposed(map: &HashMap<(i32, i32, u32), usize>) -> HashMap<(i32, i32, u32), usize> {
    map.iter().map(|(&(x, y, v), &count)| ((y, x, v), count)).collect()
}
...
fn interchromosomal_reversal_transposes() {
    ...
    .unwrap();
    assert!(!forward.is_empty(), "fixture returned no interchromosomal contacts");
...
fn cis_asymmetric_window_reflects() {
    ...
    .unwrap();
    assert!(!upper.is_empty(), "fixture returned no asymmetric cis contacts");
...
fn dense_matrix_reversal_is_transpose() {
    let path = fixture();
    let forward =
        straw::straw_as_matrix(MatrixType::Observed, Normalization::none(), &path, "1", "2", Unit::BP, 2_500_000)
            .unwrap();
    let reverse =
        straw::straw_as_matrix(MatrixType::Observed, Normalization::none(), &path, "2", "1", Unit::BP, 2_500_000)
            .unwrap();
